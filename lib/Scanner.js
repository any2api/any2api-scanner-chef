...
var Scanner = function(spec) {
  ...
  var scan = function(dir, done) {
    ...
    var tpl = [ 'cat <%= mashClass %> >> <%= tempFile %> &&',
                'echo "require \'json\'\n',
                //'Mash = Hash\n',
                'def include_attribute(foo)\nend\n',
                'def value_for_platform_family(foo)\nend\n',
                'default = node = <%= mashNew %>\n',
                //'node = <%= mashNew %>\n',
                '<%= name %> = <%= mashNew %>\n',
                '<% if (platform) { %> node[\'platform\'] = \'<%= platform %>\'\n <% } %>',
                '<% if (platformVersion) { %> node[\'platform_version\'] = \'<%= platformVersion %>\'\n <% } %>',
                '" >> <%= tempFile %> &&',
                '<% _.forEach(attrFiles, function(file) { print("cat " + file + " >> " + tempFile + " && "); }); %>',
